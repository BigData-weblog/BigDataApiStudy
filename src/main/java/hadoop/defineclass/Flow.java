...
public class Flow {
    ...
    //map最终输出的key:手机号 text  value:flowbean
    static class MyMapper extends Mapper<LongWritable, Text,Text,FlowBean>{
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            FlowBean fb=new FlowBean(Integer.parseInt(datas[datas.length-2]),Integer.parseInt(datas[datas.length-1]));
    ...
    static class MyReducer extends Reducer<Text,FlowBean,Text,FlowBean>{
        ...
        protected void reduce(Text key, Iterable<FlowBean> values, Context context) throws IOException, InterruptedException {
            ...
            for(FlowBean fb:values){
                sumupflow=sumdownflow+fb.getUpflow();
                sumdownflow=sumdownflow+fb.getDownflow();
            }
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        //输入路径：需要统计的单词的路径
        FileInputFormat.addInputPath(job,new Path("hdfs://172.16.98.186:9000/barry-test/in/in02/phone"));
